...
<div>
    ...
    @if (!empty($user_addresses))
        ...
        @foreach ($user_addresses as $count => $address)
            Posizione (lat,lon): {{ $address['geometry']->coordinates[1] }},{{ $address['geometry']->coordinates[0] }}
            <a
                href="https://www.openstreetmap.org/?mlat={{ $address['geometry']->coordinates[1] }}&mlon={{ $address['geometry']->coordinates[0] }}#map=15/{{ $address['geometry']->coordinates[1] }}/{{ $address['geometry']->coordinates[0] }}">MAPPA</a><br>
    ...
    @isset($g)
        ...
        Posizione (lat,lon): {{ $g->coordinates[1] }},{{ $g->coordinates[0] }} <a
            href="https://www.openstreetmap.org/?mlat={{ $g->coordinates[1] }}&mlon={{ $g->coordinates[0] }}#map=15/{{ $g->coordinates[1] }}/{{ $g->coordinates[0] }}">MAPPA</a><br>
        Indirizzo: {{ $ticket->location_address }}<br>
    @endisset
    Note: {{ $ticket->note }}<br>
    Link al Ticket: <a
        href="https://{{ $site_url . '/resources/tickets/' . $ticket->id }}">https://{{ $site_url . '/resources/tickets/' . $ticket->id }}</a>
</div>
